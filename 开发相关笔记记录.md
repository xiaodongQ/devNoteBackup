...
* systemctl 和 `chkconfig` 区别
    - `systemctl`命令：是一个*systemd*工具，主要负责控制systemd系统和服务管理器。
        ...
        + 由来：历史上，Linux 的启动一直采用init进程。这种方法有两个缺点。一是启动时间长。二是启动脚本复杂。
            * /etc/init.d/下面管理开机启动的服务，该目录下README文件做了历史说明(CentOS7)， 说明systemd替换了传统的init脚本："You are running a systemd-based OS where traditional init scripts have been replaced by native systemd services files"
...
* 日志频繁打印登录记录
    - [记一次系统日志中大量出现Started Session * of user root 查找和解决办法](https://www.lnmpweb.cn/archives/629)
    - 系统日志 /var/log/messages 中，总是出现"Started Session 225 of user root"
        + 查看到这个信息是存在相关规律的，一般这种情况都是在计划任务中定时执行。
        + `crontab -l` 没有正在执行的用户计划任务，怀疑是系统计划任务引起
        + 到系统计划任务目录：`/etc/cron.d` 查看，
            * 
