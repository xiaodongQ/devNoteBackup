...
* 跳过ssl验证：git config --global http.sslVerify false
...
* gcc版本可参考：[为CentOS 6、7升级gcc至4.8、4.9、5.2、6.3、7.3等高版本](https://www.vpser.net/manage/centos-6-upgrade-gcc.html)

* centos下安装gcc
    - `yum group install "Development Tools"`
    - 包含git、make、gcc/g++、ctags、automake等等
* 安装`sz`(用于xshell连接使用)
    - `yum install -y lrzsz`
    - 使用
        + `sz filename1` sz命令发送文件到本地，选择一下本地存放路径即可
        + `rz` rz命令本地上传文件到服务器，执行该命令后，在弹出框中选择要上传的文件即可
            * 直接拖动文件到xshell终端，也可以传输文件，会使用`rz`
...
* pstack
    ...
    - `pstack 进程号`，找`lock_wait`关键字，多次执行若一直阻塞在某个锁上，很有可能是死锁(不过不排除是程序执行较快，多次正常加锁解锁)

### 符号表

* [使用GDB调试将符号表与程序分离后的可执行文件](https://www.cnblogs.com/dongc/p/9690754.html)
* [strip，eu-strip 及其符号表，gdb调试strip过的程序](https://blog.csdn.net/bingqingsuimeng/article/details/50522242)
* 生成单独的符号文件
    - 生成单独的调试信息文件：
        + `objcopy --only-keep-debug a.out a.out.symbol`
    - 去除调试信息
        + `strip a.out`
        + 或者 `objcopy --strip-debug a.out`
    - 添加调试信息文件链接
...
* 加载符号表
    ...
    - 如下方式加载符号表
        + 到gdb中执行：**方法1** `symbol-file /root/workspace/cpp_path/cpp_test/a.out.symbol`
...
* `perf top [-g] -p 进程号`， 查看cpu使用率高问题
    ...
    - 结果中包含`Children`和`Self`，跟不加`-g`时展示有区别，下面是`man perf top`中的解释
       ...
       entry - usually a function (symbol). This is the value that perf shows traditionally
       and sum of all the self overhead values should be 100%.
        + `Self`：简单地叠加采样事件中该函数或者符号的调用次数，所有的`Self`开销加起来应该为`100%`
        + `Children`：通过将子函数的所有调用次数相加，这样就可以显示高级函数的总开销，即使它们没有直接执行
            * 所有`Children`开销叠加可能会超过`100%`，因为每个子函数统计时，本身就已经是它的子函数叠加之后的结果
...
* Mac下安装CentOS虚拟机
    - 通过VirtualBox
    - 安装后启动有问题(服务异常退出)，命令行尝试(失败)
        + [使用命令行启动VirtualBox虚拟机](https://www.cnblogs.com/nshuai/p/5857610.html)
        + `VBoxManage list vms`
            * "centos7" {2284e0f2-9c84-470b-a966-2be68030647d}
        + `VBoxManage startvm "centos7" --type headless`
    - 查看mac上的虚拟机日志(安装centos目录的日志)
        + 禁用声音，显示修改为VBoxVGA后正常了
    - 使用一段时间后，发现磁盘太小了，之前设置的固定8G
        + 命令行查看磁盘信息(ui查看更直观)，`VBoxManage showhdinfo centos7.vdi`
        + 网上说的`VBoxManage modifymedium`只适用于动态磁盘
        + 固定磁盘扩容感觉比较麻烦，还是重新安装了。。

* 替换yum源为阿里云
    - [CentOS 7的yum更换为国内的阿里云yum源](https://www.linuxidc.com/Linux/2019-08/160310.htm)
    - 备份：`mv /etc/yum.repos.d/CentOS-Base.repo /etc/yum.repos.d/CentOS-Base.repo.backup`
    - 下载新的CentOS-Base.repo 到/etc/yum.repos.d/：
        + `wget -O /etc/yum.repos.d/CentOS-Base.repo http://mirrors.aliyun.com/repo/Centos-7.repo`
        + 若没有域名服务(wget时会失败提示)则进行添加，`vi /etc/resolv.conf`
    ...
    - 添加EPEL(不添加前面的感觉也够用了)
        ...
        + `EPEL`
            ...
            * 我们可以很容易地通过`yum`命令从EPEL源上获取上万个在CentOS自带源上没有的软件。EPEL提供的软件包大多基于其对应的Fedora软件包，不会与企业版Linux发行版本的软件发生冲突或替换其文件
        + RHEL/CentOS系统有许多第三方源，比较流行的比如RpmForge，RpmFusion，EPEL，Remi等等。
            * 然而需要引起注意的是，如果系统添加了多个第三方源，可能会因此产生冲突——一个软件包可以从多个源获取，一些源会替换系统的基础软件包，从而可能会产生意想不到的错误
            * 已知的就有Rpmforge与EPEL会产生冲突。对于这些问题我们建议，调整源的优先权或者有选择性的安装源，但是这需要复杂的操作，如果你不确定如何操作，我们*推荐你只安装一个第三方源*
    - 清理缓存并生成新的缓存
        + `yum clean all`
        + `yum makecache`
...
* 一般操作
    - 客户机生成公钥私钥
        + `ssh-keygen`
    - 复制公钥内容到服务器上的`/root/.ssh/authorized_keys`
    - 另外可能要配置服务端的`/etc/ssh/sshd_config`
        + PermitRootLogin yes
        + RSAAuthentication yes
        + PubkeyAuthentication yes
* 但是mac下配置一直有问题
    - [mac 免密ssh登陆配置不坑指南](https://zhuanlan.zhihu.com/p/32279976)
    - 感觉是复制公钥的问题，调整为`ssh-copy-id`来上传后正常
        + `ssh-copy-id -i ~/.ssh/id_rsa.pub root@host`
    - (应该发现问题了，vim中开启了行号，command+c复制时有空格，泥煤)
    - 另外参考链接里说mac还要多一步：
...
* Off-CPU 火焰图
    - 如果是 IO 或 锁，则使用 Off-CPU火焰图，On-CPU火焰图比较适合CPU密集型
    - [用off-cpu火焰图进行Linux性能分析](https://cloud.tencent.com/developer/article/1560057)
        + 很多时候，单纯地看on-cpu的情况（什么代码在耗费CPU），并不能解决性能问题，因为有时候性能差的原因瓶颈不一定在CPU上面，而是在off-cpu的时间，比如：
        ...
        + off-cpu的状态图：
            - ![off-cpu的状态图](https://ask.qcloudimg.com/http-save/yehe-5465588/beg41cahqu.png?imageView2/2/w/1620)
    - 安装bcc-tools
        + CentOS 系统中如何安装 bcc-tools
        + 第一步，升级内核(建议>=4.1)
            * 看没升级也可以用
        + 第二步，安装 bcc-tools：
            * 安装bcc-tools：`yum install -y bcc-tools`
            * 配置PATH路径：`export PATH=$PATH:/usr/share/bcc/tools`
            * 验证安装成功：`cachestat`
                - `memleak`也在该工具包中，还有很多其他比较实用的性能工具
    - 生成步骤
        + `/usr/share/bcc/tools/offcputime -df -p 1234(进程号) 30 > out.stacks`
...
* tcpdump
    ...
    - `-v` 配合`-w`保存文件时使用，可同时显示抓到的包数量
    - `tcpdump host 192.168.1.64 and port 8000`
        + 指定过滤ip，也可执行源ip或者目的ip
        + `src 192.168.1.64`
        + `dst 192.168.1.64`
* wireshark
...
别人的wireshark笔记技巧：[Wireshark常用功能笔记](https://www.ilikejobs.com/posts/wireshark/)

* tcpdump -D 查看有哪些网卡

```sh
[root@anonymous ➜ /home/xd/workspace/libbpf-bootstrap/examples/c git:(master) ✗ ]$ tcpdump -D
1.enp4s0 [Up, Running]
...
* 技巧
    * 打标记，`ping www.baidu.com -c 1 -s 1`、`ping www.baidu.com -c 1 -s 2`、`ping www.baidu.com -c 1 -s 3`

* 常见错误
    * TCP window full，表示这个包的发送方已经把对方所声明的接收窗口耗尽了
    * TCP Zerowindow，TCP中的“win=”代表接收窗口的大小，表示这个包的发送方当前还有多少缓冲区可以接收数据。
...
* [实用工具集合之tmux](https://xiaodongq.github.io/2016/06/29/%E5%AE%9E%E7%94%A8%E5%B7%A5%E5%85%B7%E9%9B%86%E5%90%88%E4%B9%8Btmux/)
* 编译tmux报错 configure: error: "libevent not found"
    - `yum install libevent-devel`
* 复制模式
    - 按住`shift`，然后就可以鼠标选中即复制，右键即粘贴(粘贴时也要按住`shift`)
        + 不过，当一行内存在多个面板时，无法选择单个面板中的内容，这时就必须使用 tmux 自带的复制粘贴系统了
    - 复制模式
        + 进入复制模式：`ctrl+b`(默认prefix，自己改成了`ctrl+x`)，再按`[`
        + 开始复制：使用vim的j/k/h/l 或者 箭头键转到文本选择的开始/结束，按`space`(空格键)开始选中进行复制
        + 复制文字：按`enter`(回车)即复制完成(仅tmux中可使用内容)
        + 粘贴：`<prefix> + ]`

## traceroute

* [traceroute命令](https://man.linuxde.net/traceroute)
...
```sh
[root@anonymous ➜ /root ]$ /usr/bin/traceroute 192.168.1.2 -n
traceroute to 192.168.1.2 (192.168.1.2), 30 hops max, 60 byte packets
 1  192.168.1.2  5.517 ms  5.487 ms  6.739 ms
```

## mtr

`mtr`（My traceroute）是一个网络诊断工具，它结合了 `traceroute` 和 `ping` 的功能，并提供了更丰富的信息输出。mtr 允许用户追踪数据包从源主机到目标主机之间的路径，并在每个中间跳点（通常是路由器）上测量往返时间（RTT）。

与 `traceroute` 相比，mtr 提供了更详细的输出，并且会持续发送数据包以更新往返时间，从而更容易识别网络中的延迟和丢包问题。

示例：`mtr 192.168.1.2 -n`，结果是一个动态窗口，会一直发包(`Snt`计数)

```sh
                             My traceroute  [v0.92]
anonymous (192.168.1.150)                                                 2024-06-10T13:56:42+0800
Keys:  Help   Display mode   Restart statistics   Order of fields   quit
                                   Packets               Pings
 Host                              Loss%   Snt   Last   Avg  Best  Wrst StDev
 1. 192.168.1.2                    0.0%    10   82.3  65.7   1.6 128.7  44.4
```
