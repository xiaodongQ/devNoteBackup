...
vi /etc/sudoers，找到下面位置并添加用户记录(sudoers本身是没有写权限的，先chmod +w，改完再chmod -w)
...
* 日志文件监控
    ...
    - 监控项配置
        + 新建监控项，选择`Zabbix agent (active)` (对应中文界面：`Zabbix客户端(主动式)`，翻译应该翻译为代理)
        + 
