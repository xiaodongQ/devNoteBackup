...
用`SPDLOG_INFO`/`SPDLOG_TRACE` 等宏定义记录才支持下面几个信息打印：不过这些宏不能针对某个logger
...
* Samba
    ...
    - Linux操作系统提供了Samba服务，为了实现Window主机与Linux服务器之间的资源共享，Samba服务为两种不同的操作系统架起了一座桥梁，使 Linux 系统和Window系统之间能够实现互相通信。
* Samba服务组件
...
* 安装步骤参考：[CentOS服务器的目录映射为Windows磁盘驱动器](https://blog.csdn.net/u010480282/article/details/80518836)
    ...
    - 将root用户添加到samba用户当中 `smbpasswd -a root`
        ...
        + `smbclient //192.168.50.207/root(一般为配置文件中添加的路径)` 添加用户后可用本地命令测试，输入密码成功则会进入`smb: \> `
    - 修改samba配置文件
        + `/etc/samba/smb.conf`
...
* [firewall-cmd命令](https://man.linuxde.net/firewall-cmd)
    - `firewall-cmd`是Linux上新用的防火墙软件，跟iptables差不多的工具
    - firewall-cmd 是 firewalld的字符界面管理工具，firewalld是centos7的一大特性，最大的好处有两个：支持动态更新，不用重启服务；第二个就是加入了防火墙的“zone”概念
    - firewalld跟iptables比起来的好处
...
* `usermod`语法
    ...
    - e.g. `usermod -a -G samba xd`，`-G`后面接的是附加组(即用户xd加到samba组)，可以多个，逗号隔开
